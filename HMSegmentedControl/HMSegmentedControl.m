...
- (void)drawRect:(CGRect)rect {
  ...
  if (self.type == HMSegmentedControlTypeText) {
    ...
    [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
      ...
      CGFloat textWidth = 0;
      CGFloat textHeight = stringHeight;
      ...
      if (self.titleImageLayoutOrientation == HMSegmentedControlTitleImageLayoutOrientationHorizontal) {
        textYOffset = (CGRectGetHeight(self.frame)-self.selectionIndicatorHeight-textHeight)/2;
      ...
      CGRect textRect = CGRectMake(textXOffset, textYOffset, textWidth, textHeight);
  ...
  if (self.selectedSegmentIndex != HMSegmentedControlNoSegment) {
    if (self.selectionStyle == HMSegmentedControlSelectionStyleDot) {
      if (!self.selectionIndicatorDotLayer.superlayer) {
        [self setDotFrame];
        [self.scrollView.layer addSublayer:self.selectionIndicatorDotLayer];
      }
    }else if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
...
- (void)setDotFrame {
  ...
  CGFloat dotLayerWidth = CGRectGetWidth(self.selectionIndicatorDotLayer.frame);
  CGFloat dotLayerHeight = CGRectGetHeight(self.selectionIndicatorDotLayer.frame);
  CGFloat min = MIN(dotLayerWidth, dotLayerHeight);
  CGRect dotRect = CGRectMake((dotLayerWidth-min)/2, (dotLayerHeight-min)/2, min, min);
...
- (void)updateSegmentsRects {
  ...
  if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
    ...
    [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
      ...
      self.segmentWidth = MAX(combineWidth, self.segmentWidth);
    }];
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
  ...
  if (index == HMSegmentedControlNoSegment) {
    ...
    [self.selectionIndicatorDotLayer removeFromSuperlayer];
  } else {
    ...
    if (animated) {
      ...
      if (self.selectionStyle == HMSegmentedControlSelectionStyleDot) {
        if ([self.selectionIndicatorDotLayer superlayer] == nil) {
          [self.scrollView.layer addSublayer:self.selectionIndicatorDotLayer];
      ...
      self.selectionIndicatorDotLayer.actions = nil;
      ...
      self.selectionIndicatorDotLayer.actions = newActions;
      [self setDotFrame];
